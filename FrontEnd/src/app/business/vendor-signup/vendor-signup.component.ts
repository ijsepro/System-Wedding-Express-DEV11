...
export class VendorSignupComponent implements OnInit {
  ...
  ngOnInit() {
    this.rForm = this.fb.group({
      ...
      'Location':  [null, Validators.compose([Validators.required, Validators.minLength(4), Validators.maxLength(20)])],
  ...
  createUser(UserForm){
    let u={userData: UserForm.value};
     let user={email: u.userData.email, 
      fName: u.userData.name,
      userName: u.userData.username,
      password: u.userData.password,
      phone: u.userData.mobile,
      job: u.userData.Category,
